...
class NNPlayer():
    ...
    def choose_move(self, game_board):
        ...
        assert sum(flattened_board) in [0, -1, 1], "Flattened board is incorrect"
        ...
        max_index = random.choice([results[n] for n in range(len(results)) if n not in taken_spots])
        print([results[n] for n in range(len(results)) if n not in taken_spots], "b4")
