...
def run(num_first_gen, num_gen):
    ...
    max_payoff_values[0] = find_max_total_payoff_from_first_evaluation_data(first_evaluation_data, return_net)
    ...
    for n in range(2, num_gen + 1):
        ...
        file_object.write(f'{find_max_total_payoff_from_first_evaluation_data(first_evaluation_data, return_net)} \n')
...
for layer in neural_net_data['node_weights']:
    for node_num in neural_net_data['node_weights'][layer]:
        nodes[layer].append(Node(node_num))


neural_net = EvolvedNeuralNet(nodes, neural_net_info['node_weights'], neural_net_info['bias_nodes'], neural_net_info['hidden_layer_size'])
'''